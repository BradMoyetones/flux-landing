...
function OSIcon({ os, className }: { os: string; className?: string }) {
    switch (os) {
        case 'macos':
            return <Icons.AppleIcon className={className} />;
        case 'windows':
            return <Icons.WindowsIcon className={className} />;
        case 'linux':
            return <Icons.LinuxIcon className={className} />;
        default:
            return null;
    }
}
...
export function SmartDownloadButton({ assets, variant = 'hero' }: SmartDownloadButtonProps) {
    ...
    const sizeClass = variant === 'hero' ? 'px-4 py-2 md:px-8 md:py-4 text-base' : 'px-6 py-3 text-sm';
    ...
    const suggestedAsset =
        assets && platform?.suggestedAssetKey ? assets.find((a) => a.platformKey === platform.suggestedAssetKey) : null;
