...
export function getAssetPlatformKey(filename: string): string | null {
    ...
    if (lower.endsWith('.sig') || lower === 'latest.json' || lower.startsWith('source code')) {
        return null;
    }

    // macOS
    if (lower.includes('aarch64') && lower.endsWith('.dmg')) return 'aarch64_dmg';
    if (lower.includes('x64') && lower.endsWith('.dmg')) return 'x64_dmg';

    // Windows
    if (lower.endsWith('-setup.exe') || lower.endsWith('_x64-setup.exe')) return 'x64_exe';
