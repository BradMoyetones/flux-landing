...
const Search = () => {
    ...
    const results = useMemo(() => expensiveFilter(items, deferredQuery), [deferredQuery]);
...
const Form = () => {
    ...
    const errors = useMemo(() => validateForm(deferredFormData), [deferredFormData]);

    return (
        <>
            <Input value={formData.email} onChangeText={(email) => setFormData((d) => ({ ...d, email }))} />
