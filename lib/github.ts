...
function parseRelease(release: GitHubRelease): ParsedRelease {
    const assets = release.assets.map(parseAsset).filter((a): a is ParsedAsset => a !== null);
...
export async function getAllReleases(): Promise<ParsedRelease[]> {
    try {
        const res = await fetch(`${GITHUB_CONFIG.releasesUrl}?per_page=20`, FETCH_OPTIONS);
        ...
        const data: GitHubRelease[] = await res.json();
        return data.filter((r) => !r.draft).map(parseRelease);
    } catch (error) {
...
export async function getReleaseByTag(tag: string): Promise<ParsedRelease | null> {
    try {
        const res = await fetch(`${GITHUB_CONFIG.releasesUrl}/tags/${tag}`, {
            headers: {
                Accept: 'application/vnd.github+json',
                'X-GitHub-Api-Version': '2022-11-28',
            },
