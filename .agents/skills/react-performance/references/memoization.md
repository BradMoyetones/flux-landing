...
const List = ({ items }: Props) => {
    ...
    return items.map((item) => <ListItem key={item.id} id={item.id} onPress={handlePress} />);
};

// ListItem receives id as prop and calls onPress(id)
const ListItem = memo(({ id, onPress }: ItemProps) => <Pressable onPress={() => onPress(id)}>...</Pressable>);
```
...
const List = ({ items, onItemPress }) => items.map((item) => <Item key={item.id} {...item} onPress={onItemPress} />);
