...
export function PlatformCard({ asset, isSuggested }: PlatformCardProps) {
    return (
        <motion.div
            className={cn('bg-card relative rounded-2xl p-5 flex flex-col gap-4 border', {
        ...
        >
            ...
            <div className="flex items-start gap-3">
                ...
                <div className="min-w-0">
                    <h3 className="text-sm font-semibold text-foreground leading-snug">{asset.label}</h3>
            ...
            <div className="flex items-center gap-3 text-xs text-muted-foreground">
                <span className="rounded-md bg-secondary px-2 py-0.5 font-mono">{asset.format}</span>
                <span>{asset.sizeFormatted}</span>
            </div>

            {/* Download button */}
            <motion.a href={asset.downloadUrl} download whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                <Button variant={isSuggested ? 'default' : 'secondary'} size={'lg'} className="w-full">
                    <Download />
                    Descargar
                </Button>
            </motion.a>
